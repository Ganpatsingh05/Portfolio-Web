...
interface SoftSkill { id: string; name: string; level: number; icon_url?: string; sort_order?: number; created_at?: string; updated_at?: string; is_visible?: boolean; }
...
export default function SkillsPage() {
  ...
  const [editing, setEditing] = useState<Skill | SoftSkill | null>(null);
  ...
  const load = async () => {
    try {
      const [techList, softList] = await Promise.all([
        adminApi.skills.list(),
        adminApi.softSkills.list()
      ]);

      // Sort technical skills: category asc, level desc, name asc
      techList.sort((a: Skill, b: Skill) => {
      ...
      // Sort soft skills: sort_order asc, name asc
      softList.sort((a: SoftSkill, b: SoftSkill) => {
  ...
  const startEdit = (s: Skill | SoftSkill) => {
    const skillType: SkillType = 'category' in s ? 'technical' : 'soft';
    ...
    setForm({
      name:s.name,
      level:s.level,
      category: skillType === 'technical' ? ('category' in s ? s.category : '') : '',
  ...
  const save = async () => {
    ...
    try {
      if (form.skill_type === 'soft') {
        ...
        if(editing) await adminApi.softSkills.update(editing.id, payload);
        else await adminApi.softSkills.create(payload);
      } else {
        const payload = {
          name: form.name.trim(),
          level: form.level,
        ...
        if(editing) await adminApi.skills.update(editing.id, payload);
        else await adminApi.skills.create(payload);
      }
      setShowForm(false); load();
      const skillTypeLabel = form.skill_type === 'soft' ? 'Soft Skill' : 'Technical Skill';
      toast.success(editing ? 'Updated!' : 'Added!', `${skillTypeLabel} ${editing ? 'updated' : 'created'} successfully`);
    } catch(e:any){ setFormError(e.message||'Failed to save skill'); }
    finally { setSaving(false); }
  };

  const del = async (s: Skill | SoftSkill) => {
    const skillType = 'category' in s ? 'technical' : 'soft';
    ...
    if(!confirmed) return;
    try {
      if (skillType === 'soft') await adminApi.softSkills.delete(s.id);
      else await adminApi.skills.delete(s.id);
      load();
  ...
  const toggleVisibility = async (s: SoftSkill) => {
    try {
      await adminApi.softSkills.update(s.id, { is_visible: !s.is_visible });
      load();
      toast.success(s.is_visible ? 'Hidden!' : 'Visible!', `Soft skill ${s.is_visible ? 'hidden from' : 'shown on'} frontend`);
    } catch (e: any) {
      toast.error('Toggle Failed', e.message || 'Failed to toggle visibility');
    }
  };
  ...
  const technicalSkillsDisplay = technicalSkills;
  const softSkillsDisplay = softSkills;
  const activeSkills = activeSection === 'technical' ? technicalSkillsDisplay : softSkillsDisplay;

  // Group technical skills by category
  const groupedSkills = technicalSkillsDisplay.reduce((acc, skill) => {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {activeSkills.length === 0 ? (
        ...
        activeSection === 'technical' ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">{softSkillsDisplay.length} skill{softSkillsDisplay.length !== 1 ? 's' : ''}</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="p-4">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {softSkillsDisplay.map((s, index) => (
                  <div key={s.id} className={`bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600 group transition ${!s.is_visible ? 'opacity-60' : ''}`}>
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="flex-1">
                        <h4 className="font-medium text-gray-900 dark:text-white">{s.name}</h4>
                        {!s.is_visible && (
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Hidden from frontend</p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        title={s.is_visible ? 'Hide from frontend' : 'Show on frontend'}
                      >
                        {s.is_visible ? (
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showForm && (
        <div className="fixed left-0 right-0 top-16 bottom-0 bg-black/50 flex items-center justify-center z-[60] p-4">
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                >
                  ...
                  {Array.from(new Set(technicalSkills.map(s => s.category).filter(Boolean)))
